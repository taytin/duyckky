...
if (!defined('SIMPLE_AJAX_CHAT_VERSION')) define('SIMPLE_AJAX_CHAT_VERSION', '20230307');
...
if (!defined('SIMPLE_AJAX_CHAT_FILE'))    define('SIMPLE_AJAX_CHAT_FILE',    plugin_basename(__FILE__)); // = 'simple-ajax-chat/simple-ajax-chat.php';
...
function sac_i18n_init() {
	...
	foreach ($paths as $path) {
		...
		if ($loaded = load_textdomain($domain, $path)) {
			...
			return load_plugin_textdomain($domain, false, dirname(SIMPLE_AJAX_CHAT_FILE) .'/languages/');
...
function sac_require_wp_version() {
	...
	if (isset($_GET['activate']) && $_GET['activate'] == 'true') {
		...
		if (version_compare(get_bloginfo('version'), SIMPLE_AJAX_CHAT_WP_VERS, '<')) {
			...
			if (is_plugin_active(SIMPLE_AJAX_CHAT_FILE)) {
				
				deactivate_plugins(SIMPLE_AJAX_CHAT_FILE);
...
function sac_plugin_action_links($links, $file) {
	...
	if ($file == SIMPLE_AJAX_CHAT_FILE && current_user_can('manage_options')) {
